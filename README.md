...
The build needs the interop assemblies from a real BepInEx installation and goes looking for
them on its own — your Thunderstore profile whatever you called it, or a BepInEx folder beside
the game. If you keep yours somewhere unusual, or have several profiles and want a particular
one, set `LOOSELIPS_BEPINEX` to the folder that contains `interop`.
